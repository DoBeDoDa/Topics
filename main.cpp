...
const double TASK1_DIST_BACKWARD = -250.0; 
const DWORD TASK1_EXTEND_MS = 180;         
const double TASK2_POCKET_X = 400.0;       
const double TASK2_POCKET_Y = -200.0;      
...
// 3. 幾何運算與防撞核心模組
// ==========================================
bool is_path_blocked(Point start, Point end, Point obs) {
...
Point get_perpendicular_target(Point base, Point edgeA, Point edgeB, double backward_dist) {
...
Point get_ghost_ball(Point destination, Point target_ball) {
...
Point get_slanted_bank_target(Point I, Point p1, Point p2) {
...
bool get_intersection(Point ray_start, Point ray_target, Point segA, Point segB, Point &out_intersect) {
...
int main() {
    ...
    cout << "--- 撞球 AI 視覺伺服系統 (自動直擊與顆星複合版) ---" << endl;

    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) return -1;

    int id = open_connection((char*)ARM_IP, 1, arm_callback);
    if (id < 0) { cout << "[錯誤] 手臂連線失敗。" << endl; return -1; }
    
    clear_alarm(id);
    Sleep(200);
    set_motor_state(id, 1);       
    set_override_ratio(id, 50);   

    SOCKET clientSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    sockaddr_in serverAddr = { AF_INET, htons(PYTHON_PORT) };
    serverAddr.sin_addr.s_addr = inet_addr("127.0.0.1"); 

    cout << "[系統] 等待 Python 連線..." << endl;
    while (connect(clientSocket, (sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) Sleep(1000);

    char recvbuf[2048];
    bool need_camera_move = true; 
    
    // 🎯 狀態機：記錄目前的桿數，以管控第一桿與第二桿的獨立邏輯
    int shot_count = 1; 

    while (true) {
        if (need_camera_move) {
            cout << "\n[動作] 移動至拍照點..." << endl;
            ptp_axis(id, 0, (double*)CAM_JOINT); 
            ...
            if (shot_count == 1) {
                ...
                if (do_break == 'y' || do_break == 'Y') {
                    double break_joint[6] = {-2.599, -87.736, 48.629, 3.794, -50.23, 103.847};
                    
                    cout << "[動作] 移動至開球點..." << endl;
                    ptp_axis(id, 0, break_joint); 
                    while (get_motion_state(id) != 1) Sleep(50);
                    ...
                    set_digital_output(id, PNEUMATIC_DO, true); 
                    Sleep(TASK2_EXTEND_MS); 
                    set_digital_output(id, PNEUMATIC_DO, false); 
                ...
                shot_count = 2; // 推進狀態至第二桿
                need_camera_move = true; 
                continue; // 回到迴圈開頭準備評估第二桿
            }
            // ==========================================
            // 🎯 第二桿：指定關節點位獨立邏輯
            // ==========================================
                ...
                if (do_second == 'y' || do_second == 'Y') {
                    double second_joint[6] = {63.243,-93.544,63.324,0.000,-67.246,36.322};
                    
                    cout << "[動作] 移動至第 2 桿預設點..." << endl;
                    ptp_axis(id, 0, second_joint); 
                    while (get_motion_state(id) != 1) Sleep(50);
                ...
                shot_count = 3; // 推進狀態至常規 AI 視覺邏輯
                
                // 第 3 桿開始進入視覺對齊前，必須先清空舊的 Socket 影像暫存
                ...
                need_camera_move = false; 
            }
            // ==========================================
            // 🎯 第三桿以後：常規視覺伺服清除 Buffer 邏輯
            // ==========================================
            else {
                u_long bytes_available;
                ioctlsocket(clientSocket, FIONREAD, &bytes_available);
        ...
        set_tool_number(id, 1);
        int bytes = recv(clientSocket, recvbuf, 1023, 0);
        if (bytes > 0) {
            recvbuf[bytes] = '\0';
            double b1x, b1y, b2x, b2y, b3x, b3y, bwx, bwy, p1x, p1y, p2x, p2y, p3x, p3y, p4x, p4y, p5x, p5y, p6x, p6y;

            if (sscanf_s(recvbuf, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf",
                &b1x, &b1y, &b2x, &b2y, &b3x, &b3y, &bwx, &bwy, &p1x, &p1y, &p2x, &p2y, &p3x, &p3y, &p4x, &p4y, &p5x, &p5y, &p6x, &p6y) == 20) {
                
                if (bwx < -9000.0) { cout << "\r[狀態] 尋找母球中...                  " << flush; continue; }
                Point bw = { bwx, bwy };
                ...
                Point ghost_direct = get_ghost_ball(destination, target_arm);
                bool direct_path_blocked = false;

                for (const auto& obs : obs_list) {
                    if (is_path_blocked(bw, ghost_direct, obs)) direct_path_blocked = true;      
                    if (is_path_blocked(target_arm, destination, obs)) direct_path_blocked = true; 
                }
                if (is_path_blocked(target_arm, destination, bw)) direct_path_blocked = true;      
                ...
                if (angle_deg > 90.0 || direct_path_blocked) {
                    ...
                    Point mirrored_pocket = get_slanted_bank_target(destination, rail_A, rail_B);
                    Point mirrored_target = get_slanted_bank_target(target_arm, rail_A, rail_B);
                    Point ghost_bank = get_ghost_ball(mirrored_pocket, mirrored_target);

                    Point pt_wall;
                    bool bank_route_safe = false;

                    if (get_intersection(bw, ghost_bank, rail_A, rail_B, pt_wall)) {
                        bool current_bank_blocked = false;

                        for (const auto& obs : obs_list) {
                            if (is_path_blocked(bw, pt_wall, obs)) current_bank_blocked = true;
                            if (is_path_blocked(pt_wall, target_arm, obs)) current_bank_blocked = true;
                            if (is_path_blocked(target_arm, destination, obs)) current_bank_blocked = true;
                        }
                        if (is_path_blocked(target_arm, destination, bw)) current_bank_blocked = true;
                ...
                if (confirm == 'r' || confirm == 'R') { need_camera_move = true; continue; } 
                else if (confirm != 'y' && confirm != 'Y') continue; 

                cout << "[動作] 平移至預備點..." << endl;
                ptp_pos(id, 0, ready); while (get_motion_state(id) != 1) Sleep(50); 
                
                cout << "[動作] 直線下降至傾斜擊球高度..." << endl;
                lin_pos(id, 0, 0, down); while (get_motion_state(id) != 1) Sleep(50);
                
                cout << "\n[視覺伺服] 啟動 align.py 進行相機微調對齊..." << endl;
                system("start cmd /k C:\\Users\\10003\\miniconda3\\envs\\yolo-env1\\python.exe align.py"); 
                
                SOCKET alignSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
                sockaddr_in alignAddr = { AF_INET, htons(ALIGN_PORT) };
                alignAddr.sin_addr.s_addr = inet_addr("127.0.0.1");

                cout << "[視覺伺服] 正在連線至對齊伺服器" << flush;
                bool align_connected = false;
                
                for (int i = 0; i < 10; i++) {
                    if (connect(alignSocket, (sockaddr*)&alignAddr, sizeof(alignAddr)) != SOCKET_ERROR) {
                ...
                if (align_connected) {
                    ...
                    set_tool_number(id, 1);
                    set_override_ratio(id, 5);  

                    while (!align_done) {
                        int r_bytes = recv(alignSocket, alignBuf, 511, 0);
                        if (r_bytes > 0) {
                            alignBuf[r_bytes] = '\0';
                            ...
                            if (sscanf_s(alignBuf, "%lf", &pixel_error) == 1 && pixel_error > -9000.0) {
                                ...
                                if (abs(pixel_error) <= 3.0) {
                                    ...
                                    lin_rel_pos(id, 0, 0.0, &rel_move[0]); 
                    ...
                    closesocket(alignSocket);
                } else {
                    cout << "[警告] 無法連線至 align.py，跳過二次微調。" << endl;
                }

                set_override_ratio(id, 50); 
                set_tool_number(id, 0);
